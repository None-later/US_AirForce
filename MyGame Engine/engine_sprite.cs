...
namespace TheGame
{
    public class Engine_Sprite: ICloneable
    {
        ...
        public void Add(Engine_Animation animation)
        {
        ...
        public Engine_Animation Animation
        {
            get
            {
        ...
        public void Render()
        {
            ...
            if (m_ShowAllPixels)
                Engine_Game.Device.RenderState.AlphaTestEnable = false;
            else
                Engine_Game.Device.RenderState.AlphaTestEnable = true;

            if (m_Opacity >= 100)
                Engine_Game.D3DSprite.Begin(SpriteFlags.None);
            else
                Engine_Game.D3DSprite.Begin(SpriteFlags.AlphaBlend);

            int alpha = (m_Opacity * 255) / 100;
            
            Engine_Game.D3DSprite.Draw2D(m_Animations[m_CurrentAnimationIndex].CurrentPicture.Texture, new Point(0, 0), 0, new Point(0, 0), Color.FromArgb(alpha, 255, 255, 255));

            Engine_Game.Device.Transform.World = m_LocalMatrix;

            Engine_Game.D3DSprite.End();
        }
        ...
        {
            return new Engine_Sprite(this);
        }

        #region PrivateData

        List<Engine_Animation> m_Animations = new List<Engine_Animation>();
